...
const TEST_FLOW_STEPS = [
  ...
  },
  {
    ...
    details: [
      ...
      'Article browsing and business directory',
    ...
    return (
      <div className="rounded-xl bg-amber-500/10 border border-amber-500/30 p-6 text-amber-800">
        ...
        <p className="text-sm opacity-90 mb-2">
          From the <code className="bg-black/10 px-2 py-1 rounded">river-side-herald</code> directory, run:
        </p>
      ...
      <main className="container mx-auto px-4 py-10 sm:py-16">
        <div className="max-w-4xl mx-auto">
          ...
          <section className="mb-12">
            ...
            {coverageLoading ? (
              <div className="rounded-xl bg-white/50 border border-gray-200 p-6 text-gray-600">
                Loading coverage…
              </div>
            ) : (
              ...
              {TEST_FLOW_STEPS.map((step, i) => (
                ...
                >
                  <div className="flex items-start gap-4">
                    <span className="flex-shrink-0 w-10 h-10 rounded-full bg-primary-500/20 flex items-center justify-center text-gray-900 font-bold">
                      {i + 1}
                    </span>
                    <div className="flex-1 min-w-0">
                      <h3 className="text-lg font-semibold text-gray-900 mb-1">{step.title}</h3>
                      <p className="text-sm text-gray-600 mb-2">{step.tool}</p>
                      <p className="text-gray-800/90 mb-4">{step.description}</p>
          ...
          <section className="mt-12 rounded-2xl bg-white/50 border border-gray-200 p-6 backdrop-blur-sm">
            ...
            <div className="space-y-2 font-mono text-sm">
              <p className="text-gray-800">
                <span className="text-gray-500">$</span> npm run test
                <span className="text-gray-500 ml-2">— Single run</span>
              </p>
              <p className="text-gray-800">
                <span className="text-gray-500">$</span> npm run test:watch
                <span className="text-gray-500 ml-2">— Watch mode</span>
              </p>
              <p className="text-gray-800">
                <span className="text-gray-500">$</span> npm run test:coverage:public
                <span className="text-gray-500 ml-2">— Generate coverage and copy to Testing page</span>
              </p>
              <p className="text-gray-800">
                <span className="text-gray-500">$</span> npm run test:e2e
                <span className="text-gray-500 ml-2">— Cypress E2E (headless)</span>
              </p>
              <p className="text-gray-800">
                <span className="text-gray-500">$</span> npm run test:e2e:open
                <span className="text-gray-500 ml-2">— Cypress E2E (interactive)</span>
              </p>
            </div>
          </section>
