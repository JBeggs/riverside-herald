...
import ArticlePrintLabelCard from '@/components/articles/ArticlePrintLabelCard'
import {
  buildDynamicPageCss,
  LABEL_PAPER_OPTIONS,
  readStoredPaperSize,
...
const PRINT_LABEL_BODY_CLASS = 'article-print-label-active'

type PrintLabelClientProps = ArticlePrintLabelData & {
  articleSlug: string
}

export default function PrintLabelClient({ articleSlug, ...label }: PrintLabelClientProps) {
  ...
  const handlePrint = useCallback(async () => {
    setPrinting(true)
    try {
      const imgs = Array.from(document.querySelectorAll<HTMLImageElement>('.article-print-label-root img'))
      await Promise.all(
        imgs.map((img) =>
          Promise.race([
            (async () => {
  ...
  return (
    <div className="article-print-label-page min-h-screen bg-[#f3f1ed] px-4 py-6">
      ...
      <div data-print-chrome className="no-print mx-auto mb-6 max-w-lg space-y-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            ...
            disabled={printing}
            className="inline-flex items-center gap-2 rounded-full bg-[#991B1B] px-5 py-2 text-sm font-semibold text-white disabled:opacity-60"
          >
